...
int main(int argc, char* argv[])
{
    ...
    // Prefer the "offscreen" platform so tests do not touch a real display.
    // "minimal" skips font rendering (tofu glyphs), which makes widget grabs
    // useless for reviewing the multisig wizard.
#if defined(WIN32)
    if (getenv("QT_QPA_PLATFORM") == nullptr) _putenv_s("QT_QPA_PLATFORM", "offscreen");
#else
    setenv("QT_QPA_PLATFORM", "offscreen", 0 /* overwrite */);
#endif
...

    {
        ...
        MultisigWizardTests test7(app.node());
        num_test_failures += QTest::qExec(&test7);

        VaultHardwareTests test8(app.node());
        num_test_failures += QTest::qExec(&test8);

        VaultRenewalTests test9;
