...
CBlockIndex* CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, CBlockIndex* pindexStop, const WalletRescanReserver &reserver, bool fUpdate)
{
    ...
    {
        fAbortRescan = false;
        ShowProgress(_("[%s] Rescanning...", GetName()), 0); // show rescan progress in GUI as dialog or on splashscreen, if -rescan on startup
        ...
        while (pindex && !fAbortRescan)
        {
            if (pindex->nHeight % 100 == 0 && dProgressTip - dProgressStart > 0.0) {
                ...
                ShowProgress(_("[%s] Rescanning..."), GetName(), std::max(1, std::min(99, (int)((gvp - dProgressStart) / (dProgressTip - dProgressStart) * 100))));
            }
            if (GetTime() >= nNow + 60) {
                nNow = GetTime();
                LOCK(cs_main);
        ...
        ShowProgress(_("[%s] Rescanning..."), GetName(), 100); // hide progress dialog in GUI
    }
    return ret;
}
