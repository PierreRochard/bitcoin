...
bool WalletInit::Verify() const
{
    ...
    if (gArgs.IsArgSet("-walletdir")) {
        ...
        if (error || !fs::exists(canonical_wallet_dir)) {
            return InitError(strprintf(_("Specified -walletdir \"%s\" does not exist"), canonical_wallet_dir.string()));
        } else if (!fs::is_directory(canonical_wallet_dir)) {
            return InitError(strprintf(_("Specified -walletdir \"%s\" is not a directory"), canonical_wallet_dir.string()));
